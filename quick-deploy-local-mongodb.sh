...
################################################################################
# BELLEZAPP - DigitalOcean Quick Deploy (MongoDB Local)
# 
# Este script automatiza TODO el proceso de despliegue en 2 comandos
# 
# Prerrequisitos:
...
#
# Uso:
# ./quick-deploy.sh <IP_DROPLET> <JWT_SECRET>
#
# Ejemplo:
# ./quick-deploy.sh 192.168.1.100 "abc123def456..."
#
# Generar JWT_SECRET:
# openssl rand -hex 32
#
...
if [ $# -lt 2 ]; then
    echo "❌ Uso: $0 <IP_DROPLET> <JWT_SECRET>"
    echo ""
    echo "Ejemplo:"
...
echo "║          (MongoDB Local en Droplet)                           ║"
echo "╚════════════════════════════════════════════════════════════════╝"
echo ""
echo "📊 Parámetros:"
echo "  Droplet IP: $DROPLET_IP"
echo "  MongoDB: mongodb://localhost:27017/bellezapp (Local)"
...
EOF
echo -e "${GREEN}✅ Paquetes instalados (incluido MongoDB)${NC}"
echo ""
...
echo "    pm2 logs"
echo ""
echo "6️⃣  BACKUPS MONGODB:"
echo "    ssh root@$DROPLET_IP"
echo "    mkdir -p /backups/mongodb"
echo "    mongodump --out=/backups/mongodb/\$(date +%Y%m%d)"
echo ""
echo "🔍 Para ver estado:"
echo "    ssh root@$DROPLET_IP 'pm2 list'"
echo "    ssh root@$DROPLET_IP 'systemctl status mongod'"
echo ""
